...
import { Eye, EyeOff, Mail, Lock, AlertCircle, ArrowLeft, GraduationCap, CheckCircle, Shield } from "lucide-react";
import Link from "next/link";

const DEMOS = [
  { email:"rahul@college.edu",   name:"Rahul Kumar"   },
  { email:"arjun@college.edu",   name:"Arjun Mehta"   },
  { email:"priya@college.edu",   name:"Priya Sharma"  },
];
...
export default function StudentLoginPage() {
  ...
  async function handleLogin(e) {
    ...
    const found = DEMOS.find(s => s.email.toLowerCase() === email.toLowerCase());
    sessionStorage.setItem("studentName",  found?.name || "Student");
  ...
  return (
    <div className="min-h-screen flex overflow-hidden" style={{ background:"var(--bg)" }}>
      ...
      <div className="hidden lg:flex flex-col justify-between w-[420px] shrink-0 p-10 relative overflow-hidden"
        style={{ background:"linear-gradient(160deg,#14532D 0%,#15803D 50%,#16A34A 100%)" }}>
        ...
        <div className="relative space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold"
            ...
            <GraduationCap size={11}/> Student Exam Portal
          </div>
          <h2 className="text-3xl font-extrabold text-white leading-tight">Online<br/>Examination</h2>
          <p className="text-sm leading-relaxed" style={{ color:"rgba(255,255,255,0.75)" }}>
            AI-monitored secure examination platform. Fair, transparent and seamless.
          </p>
          <div className="space-y-2 pt-2">
            {["View your scheduled assessments","AI-monitored secure exam environment","Real-time question navigation","Instant submission confirmation"].map(f=>(
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8 relative">
        ...
        <div className="w-full max-w-sm">
          ...
          <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.4 }}>
            <div className="mb-8">
              <h1 className="text-2xl font-extrabold mb-1" style={{ color:"var(--text-primary)" }}>Student Login</h1>
              <p className="text-sm" style={{ color:"var(--text-muted)" }}>Sign in to access your examinations</p>
            </div>
            ...
            <div className="mt-6">
              ...
              {DEMOS.map(s => (
                <motion.button key={s.email} whileHover={{ scale:1.01 }} type="button"
                  onClick={()=>{ setEmail(s.email); setPassword("demo123"); }}
                  className="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-left mb-2 transition-all"
                  style={{ background:"var(--bg-subtle)", border:"1px solid var(--border)" }}
                  onMouseEnter={e=>e.currentTarget.style.borderColor="rgba(21,128,61,0.25)"}
